...
최소 유효 문서의 뼈대, `examples/` 4개가 공통으로 지키는 관용구(평평한 `nodes` 맵과
...
이 스킬을 쓸 때 가장 먼저 알아야 할 제약이다. **v0.1이 지원하는 노드 타입은 `frame`과
`text` 둘뿐이다.** `image`, `button`, `input`, `component` 같은 타입은 없다.

그래서 "버튼 넣어줘", "여기 이미지 하나" 같은 요청이 오면 에이전트는 타입을 지어내지 않고
프레임과 텍스트로 근사한다. 그리고 **근사했다는 사실을 사용자에게 알린다.** 조용히 넘어가면
사용자는 버튼 노드가 실제로 생긴 줄 안다. 근사가 마음에 들지 않으면 그 자리에서 다시
논의하면 된다.

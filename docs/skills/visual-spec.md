...
- [ ] **Ticket Compiler / 컴포넌트별 파일 분리 생성** — 지금은 화면 하나 = 파일 하나다.
      IR을 컴포넌트 단위로 쪼개 의존성 순서(자식 먼저)대로 여러 파일을 생성해야 한다
      (`component-architecture.md`의 `vsb-ticket-compiler`).
- [ ] **상대 경로 import 규칙** — 컴포넌트가 여러 파일로 쪼개지면 서로 import해야 하는데,
      `@/` 별칭 금지·상대 경로만 허용해야 한다 (export 폴더가 어느 프로젝트에서도 그대로
      동작하게 하기 위함). Ticket Compiler 작업과 묶어서 간다.

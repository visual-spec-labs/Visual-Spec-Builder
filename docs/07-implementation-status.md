...
> 이미 쓰던 패턴(키 상수 분리, try/catch로 접근 실패 흡수)을 그대로 따랐다. **알려진 한계** —
> 저장이 500ms 디바운스라, 그 안에 새로고침하면 마지막 변경은 유실된다(`App.tsx`의
> `useSpecAutosave` 주석 참고). **이 브랜치는 이 문단을 쓴 시점엔 아직 `develop`에 머지되지
> 않았다.**
>

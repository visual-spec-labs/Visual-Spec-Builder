...
|---|---|---|---|
| CLI | `npx visual-spec init` / `npx visual-spec` | **부분(2026-09-09, 이슈 #42·#104)** | `package.json` 에 `bin` 필드가 생겼고(`{ "visual-spec": "./bin/visual-spec.mjs" }`), `bin/visual-spec.mjs` 가 진입점이다(빌드 없는 순수 Node 스크립트 — `scripts/generate-types.mjs` 와 같은 방식). **`init`·`skills` 두 명령이 있다.** `skills` 는 이 패키지의 `skills/` 5종을 `.claude/skills/` 로 복사한다 — `init` 과 달리 **덮어쓴다**(스킬은 이 도구가 배포하는 콘텐츠라 "다시 설치"가 "최신으로 맞춘다"는 뜻이어야 한다는 판단, 이슈 #104), 다만 파일 내용이 같으면 쓰지 않는다. 인자 없는 `npx visual-spec`(GUI 실행)은 이슈 #105로 분리해 아직 없다 |
...
| 테스트 | `test/editor-store.test.ts`(49) · `test/canvas-layout.test.ts`(30) · `test/validate.test.ts`(21) · `test/apply-command.test.ts`(20) · `test/compile-tickets.test.ts`(18) · `test/project-spec.test.ts`(15) · `test/border-patch.test.ts`(14) · `test/resolution-presets.test.ts`(13) · `test/canvas-selection.test.ts`(12) · `test/radius-patch.test.ts`(12) · `test/selection-rect.test.ts`(12) · `test/cli-skills.test.ts`(7) · `test/cli-init.test.ts`(6) · `test/schema.test.ts`(10) · `test/fit-zoom.test.ts`(10) · `test/view-store.test.ts`(9) · `test/effect-patch.test.ts`(8) · `test/export-spec.test.ts`(6) · `test/history.test.ts`(6) · `test/shadow-patch.test.ts`(5) · `test/create-node.test.ts`(4) · `test/node-id.test.ts`(4) · `test/public-api.test.ts`(4) · `test/resolve-import-parent.test.ts`(4) · `test/load-spec.test.ts`(3) · `test/home-preview.test.ts`(3) · `test/navigation-store.test.ts`(3) · `test/tool-store.test.ts`(3) | **28파일 311케이스 전부 통과** (2026-09-11, 이슈 #104 브랜치 · PR #109(이슈 #101) 머지된 `develop` 2fb4115 위로 리베이스한 기준 `pnpm test` 확인 — `cli-skills.test.ts`의 7케이스(이슈 #104), `editor-store.test.ts`의 노드 삭제 관련 9케이스(PR #109)가 새로 들어왔다) |
...
- **해소됨(2026-09-09, 이슈 #40)** — `editorStore.setNodeField`·`setPageField` 가 이제 내부적으로
  ...
  `insertNode`(Import·도구 모음)가 아직 이 경로 밖이라는 것과, Undo/Redo를 부를 UI가 없다는 것 두
  가지다(1절 표 아래·2절 Undo/Redo 행 참고).
...
pnpm test            # 28파일 311케이스 통과 (2026-09-11, 이슈 #104 브랜치 · PR #109(이슈 #101) 머지 후 확인)
```

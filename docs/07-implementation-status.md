...
| `src/features/editor/store/editorStore.ts` | **스키마 v0.2 로 넓어졌다** — `EditorState` 가 `spec: ProjectSpec` 과 **`activePageId`** 를 들고, 초기값은 `migrateV01(seedSpec)` 이다. 계약 멤버는 **16개**다(2026-09-18 3차 재실측 — `EditorState` 인터페이스 전문(`store/editorStore.ts:72`–`:180`)을 세고 [EDITOR_STORE_CONTRACT.md](EDITOR_STORE_CONTRACT.md) §2 "계약의 전부 (16개)" 의 표 16행과 1:1 대조했다. 목록·개수 일치. 이슈 #131 은 `history` 의 **타입만** 바꿔서 개수는 그대로였고, 인터페이스 줄 범위만 주석이 늘며 `:42`–`:140` 에서 옮겨갔다): `spec` · `activePageId` · `selectedId` · **`history`**(**프로젝트 하나짜리** undo 스택 — 보통 직접 읽지 않고 `undo`/`redo` 가 쓴다. 2026-09-18·이슈 #131 로 `Record<PageId, HistoryState<ScreenSpec>>`(페이지별 독립 스택)에서 `HistoryState<EditorSnapshot>` 으로 바뀌었고 `EditorSnapshot = { spec, activePageId }` 다) · `select` · **`selectPage`**(활성 페이지 전환 + 선택 해제 — 페이지마다 `root`·`cardA` 같은 id 가 겹쳐서다) · `setNodeField` · **`setPageField`**(페이지 이름·해상도) · **`addPage`** · **`removePage`**(마지막 한 장은 지우지 않는다 — `pages` 가 비면 스키마의 `minProperties: 1` 이 깨진다. 활성 페이지를 지우면 같은 자리의 이웃으로 옮겨 간다) · `loadSpec` · `insertNode` · **`removeNode`** · **`moveNode`** · **`undo`** · **`redo`**. **여기 2026-09-08 값으로 "11개"라고 적혀 있던 것은 낡은 숫자였다** — 계약은 그 뒤 11 → 14 → 15 → 16 으로 세 번 늘었고 전부 이 브랜치 이전 커밋이다(늘어난 다섯은 `history`·`removeNode`·`moveNode`·`undo`·`redo`). **노드를 다루는 `select`·`setNodeField`·`insertNode` 는 시그니처가 예전 그대로다** — 스토어가 안에서 `spec.pages[activePageId]` 를 찾으므로 호출자는 페이지를 신경 쓰지 않는다. `loadSpec` 은 v0.1(`VisualSpec`)과 v0.2(`ProjectSpec`)를 모두 받아 v0.1 이면 `migrateV01` 로 넓힌다. `insertNode` 는 `parentId` 가 없거나 frame 이 아니면 아무 것도 하지 않는다 — 유효한 frame id 를 고르는 책임은 호출자에게 있다. **`selectPage`·`addPage`·`removePage` 에도 이제 UI 호출자가 있다**(2026-09-18 3차 재실측 — `git grep` 상 `src/` 에서 스토어 정의 말고 이 셋을 부르는 곳은 `ui/LayerTree.tsx` 뿐이다: `:339` `selectPage` · `:353` `removePage` · `:529` `addPage`(`:532` 는 낡은 줄 번호였다). 위 `ui/LayerTree.tsx` 행 참고). **`undo` 의 의미가 2026-09-18(이슈 #131)에 바뀌었다** — "활성 페이지의 마지막 편집 한 단계"가 아니라 **"프로젝트 전체의 마지막 편집 한 단계"**이고, 되돌린 편집이 다른 페이지에 있었으면 **캔버스가 그 페이지로 함께 옮겨 간다**(스냅숏이 `activePageId` 를 들고 있어서다 — 안 그러면 되돌린 변화가 화면 밖에서 조용히 일어나고, `addPage` 를 되돌릴 때는 보고 있던 페이지가 사라지므로 아예 깨진다). **`insertNode`·`addPage`·`removePage` 도 이제 history 에 쌓인다**(같은 이슈) — `insertNode` 는 `createNode` Command 를 거치고(`:386`), `addPage`(`:330`)·`removePage`(`:365`)는 Command 로 표현되지 않아(`applyCommand` 는 화면 한 장만 알고 "페이지가 여러 장"을 아예 모른다) 스토어가 직접 스냅숏을 얹는다. **스냅숏을 만드는 자리는 `makeSnapshot(spec, activePageId)`(`:203`) 하나로 모아 뒀고**, `applied()`(`:234`) 헬퍼가 "Command 적용 → 결과가 원본과 같은 참조면 `null`(빈 단계를 안 쌓는다) → 스냅숏 한 단계" 순서를 담는다. `present` 가 낡을 수 있는 경로가 없어져 옛 `reconciledHistory` 는 사라졌다 | — (`test/editor-store.test.ts` **70케이스**, 2026-09-18 3차 실측) |
...
스토어를 직접 부른다), **`src/features/editor/ui/Canvas.tsx:556`·`:561`**(리사이즈 핸들 드래그, PR #99).
`src/` 의 나머지 `setNodeField` 히트는 스토어의 정의(`store/editorStore.ts:112`)와 주석이다.

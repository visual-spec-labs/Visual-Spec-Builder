...
> `pnpm test` 의 **30파일 336케이스**는 이 작업의 base 인 `develop` 7803cdf(3c3e434 에
> PR #111/이슈 #92 가 머지된 커밋) 위에서 잰 값이다 — 위 #92 행과 같은 수치이고, 이 작업
> 자체는 테스트를 늘리거나 줄이지 않았다. `main` 을 지워도 `pnpm run build` 가 깨지지
> 않는지 확인하려고 돌린 것이다. 그 뒤 이 브랜치에 `develop` 1b3d5f3 을 머지해
> #105(CLI GUI 실행)가 들어왔고, **그 #105 행은 31파일 339케이스를 적고 있다 — 그 값은
> 우리가 재지 않았으므로 #45 행으로 끌어오지 않는다.** 확인하지 않은 항목의 날짜·수치는
> 올리지 않는다는 이 문서의 규율 그대로다. **`private: true` 는 그대로다 — npm 배포는
> 여전히 불가능하고, 공개 배포 전환은 별도 이슈로 분리했다.** 5.1 밖의 어떤 절도
> 재검증하지 않았으므로 다른 항목의 날짜는 올리지 않았다.
>
